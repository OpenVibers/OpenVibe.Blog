 ...
 *       A first-party service with a Network client-credentials token for audience openvibe.blog.
 *       X-OV-Subject names the person it acts for; X-OV-Origin: ai marks AI output (OpenVibe.AI's
 *       blog.draft_post), which is never attributed to that person as author — it is AI-generated,
 *       starts as a draft and needs a person's review before it can be published.
...
function createViewerResolver({ auth, config, people }) {
    async function fromServiceToken(req, token) {
        ...
        if (!firstParty && claims.env !== undefined && claims.env !== 'production') {
            throw new ViewerError(401, 'token.sandbox_refused', 'sandbox tokens are not accepted by openvibe.blog');
        }
    ...
    async function fromUserToken(token) {
        ...
        // Staff powers come from the contracts staff map (ADR-022): moderation on any blog, and the official blog.
        return { kind: 'user', subject, staff: staffMap.can(claims, 'staff.content.moderate'), editorial: staffMap.can(claims, 'staff.editorial.manage'), origin: 'user', user: claimsToUser(claims), token };
    }
 ...
 * judged by membership in the domain layer.
 */
function guard(cap) {
    return (req, res, next) => {
        const v = req.viewer;
