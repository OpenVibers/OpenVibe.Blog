 ...
 * BLOG_FORM_SECRET (a random per-process key when unset, so forms opened before a restart must be
...
function csrfToken(config, viewer) {
    ...
    return crypto.createHmac('sha256', config.formSecret || fallback).update(`blog-form:${viewer.subject}`).digest('base64url').slice(0, 32);
}
