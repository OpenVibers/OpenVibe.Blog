...
/**
 * Comments are OpenVibe.Community threads, referenced — never copied. A public post's thread is the
 * one Community resolves for EntityRef { service: 'blog', type: 'post', id } (openvibe-publishing/
 * discussion stores only the thread id, in blog_post_discussion_refs). Pages read the thread from
 ...
 *
 * Only PUBLIC posts get threads: a Community thread is readable by anyone who has its id, so a
 * members-only or private post must not have one.
 *
 * Calls with Blog's service token (audience openvibe.community):
 *   community.comment.write     resolve a thread; comment as the signed-in member (X-OV-Subject)
 *   community.comment.moderate  (optional) hide the thread of a post that was deleted or made non-public
 */
...
function createCommunity({ store, config, fetchImpl = globalThis.fetch }) {
    ...
    const refFor = (post, label) => ({ service: 'blog', type: 'post', id: post.id, ...(label ? { label: String(label).slice(0, 200) } : {}) });
    ...
    return {
        ...
        /**
         * Best effort: hide a post's thread when the post is deleted or stops being public, show it
        ...
        async setThreadVisibility(post, visibility, ctx) {
            ...
            try {
                ...
                console.warn(`[Blog] could not set the comment thread of ${post.id} to ${visibility}: ${err.message}`);
                return false;
            }
        },
    };
