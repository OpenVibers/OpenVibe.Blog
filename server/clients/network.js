...
/**
 * Who a subject is, for display (author bylines, author pages, member lists).
 *
 * Blog stores people as Network subject ids (usr_…) only. Names come from the signed-in member's
    ...
    return {
        ...
        async many(subjects) {
            ...
            if (missing.length && tokens) {
                let timer;
                await Promise.race([
                    resolve(missing).catch((err) => console.warn('[Blog] identity resolve failed:', err.message)),
        ...
        /** The subject a username belongs to, from people Blog has seen. */
        byUsername(username) {
