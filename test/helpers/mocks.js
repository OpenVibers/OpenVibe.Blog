'use strict';
/**
 * In-process stand-ins for Blog's neighbours, with a real RS256 key pair:
 ...
 *   Media      /api/v2/<app>/objects/:id → 200 | 404 | 410 by a table the test controls
 * userToken()/serviceToken() mint the tokens browsers and services present to Blog.
 */
...
async function startMedia({ network }) {
    const objects = new Map();   // id → 'ready' | 'deleted' | 'gone' (404)
    let down = false;
    const srv = await listen((req, raw, json) => {
        if (down) return json(503, { code: 'down' });
        const m = req.url.match(/^\/api\/v2\/([a-z0-9-]+)\/objects\/(med_[0-9A-Z]+)$/);
        if (!m) return json(404, { code: 'route.not_found' });
        const token = String(req.headers.authorization || '').slice(7);
        const v = serviceAuth.verifyServiceToken(token, { publicKey: network.publicPem, issuer: network.url, audience: 'openvibe.media' });
        if (!v.ok || !(v.claims.cap || []).includes('media.object.read')) return json(403, { code: 'capability.denied' });
        const st = objects.get(m[2]);
        if (!st || st === 'gone') return json(404, { code: 'media.object.not_found' });
        if (st === 'deleted') return json(410, { code: 'media.object.deleted' });
        return json(200, { id: m[2], lifecycle_status: 'ready' });
    });
    return { ...srv, objects, setDown: (v) => { down = v; } };
}

module.exports = { startNetwork, startCommunity, startMedia, listen };
