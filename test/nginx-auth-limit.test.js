'use strict';
// deploy/nginx/openvibe.blog.conf: the shared navbar asks /auth/me on every page view, so the session probe has its own location on
...
assert.match(probe, /limit_req zone=ovblog_api burst=30 nodelay;/);
assert.match(auth, /limit_req zone=ovblog_auth burst=10 nodelay;/);
